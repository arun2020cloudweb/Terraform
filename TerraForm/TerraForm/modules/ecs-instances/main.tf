...
resource "aws_launch_configuration" "ecs_instance_api" {
  name_prefix = "api-${var.cluster_name}-instance-"
  instance_type = "${var.instance_type}"
  key_name = "${var.key_pair_name}"
  iam_instance_profile = "${var.instance_profile_name}"
  security_groups = ["${var.security_group_ecs_instance_id}"]
...
resource "aws_autoscaling_group" "api_ecs_cluster" {
  name = "api-${var.cluster_name}-instances"
  min_size = "${var.asg_min}"
  max_size = "${var.asg_max}"
  launch_configuration = "${aws_launch_configuration.ecs_instance_api.name}"
  vpc_zone_identifier = ["${split(",", var.ecs_cluster_subnet_ids)}"]
  health_check_type = "EC2"
  target_group_arns = ["${var.target_group_arn}"]
  ...
  tag {
    key = "Name"
    value = "api-${var.cluster_name}-instance"
    propagate_at_launch = true
  }
}
...
resource "aws_autoscaling_policy" "api_scale_up" {
  name = "api-${var.cluster_name}-instances-scale-up"
  scaling_adjustment = 1
  adjustment_type = "ChangeInCapacity"
  cooldown = 300
  autoscaling_group_name = "${aws_autoscaling_group.api_ecs_cluster.name}"
}

resource "aws_autoscaling_policy" "api_scale_down" {
  name = "api-${var.cluster_name}-instances-scale-down"
  scaling_adjustment = -1
  adjustment_type = "ChangeInCapacity"
  cooldown = 300
  autoscaling_group_name = "${aws_autoscaling_group.api_ecs_cluster.name}"
